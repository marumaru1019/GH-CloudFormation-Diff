...
## CloudFormationレビュールール

このレビュールールに従っているかをチェックすること。

### リソース設計
- CloudFormationのベストプラクティスに準拠しているか
- リソースの命名規則が統一されているか（PascalCase推奨）
- 適切なタグ設定がされているか
- リソース間の依存関係が正しく定義されているか

### セキュリティ
- IAMロールとポリシーが最小権限の原則に従っているか
- セキュリティグループのポートが必要最小限に絞られているか
- S3バケットのパブリックアクセスが適切に制御されているか
- データベースやストレージの暗号化が有効になっているか
...
### 運用性
- バックアップとリストア戦略が考慮されているか
- ログ出力設定が適切か
- モニタリング設定が十分か
- ディザスタリカバリが考慮されているか

### 変更影響の確認項目
- 既存リソースへの影響（更新、削除、置換）を確認すること
- データ損失の可能性がある変更については特に詳細にレビューすること
- ダウンタイムが発生する可能性のある変更を指摘すること
- ロールバック手順について確認すること
